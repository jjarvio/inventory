 ...
 * - Woo-kategoria = Snipe-kategorian alkuosa (ilman suffixia)
...
function category_is_for_sale(string $category): bool
{
    ...
    $suffix = mb_strtolower(trim($SALES_SUFFIX), 'UTF-8');
...
function derive_woo_category_name(string $snipeCategory): string
{
    ...
    $name = trim($snipeCategory);
    if ($name === '') {
        return '';
    }

    $suffix = preg_quote(trim($SALES_SUFFIX), '/');
    return trim((string) preg_replace('/' . $suffix . '$/iu', '', $name));
}
