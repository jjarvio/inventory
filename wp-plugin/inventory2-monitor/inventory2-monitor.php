...
function inv2_render_admin_page(): void
{
    ...
    if ($runResult || $cleanupResult) {
        echo '<h2 class="title">Suoritettu toiminto</h2>';
        echo '<div class="postbox" style="max-width:980px;">';
        echo '<div class="postbox-header"><h2 class="hndle">Toimintaloki</h2></div>';
        echo '<div class="inside">';
        if ($runResult) {
            $status = !empty($runResult['ok']) ? 'OK' : 'VIRHE';
            echo '<p><strong>Cron-ajo:</strong> ' . esc_html($status) . ' (exit code ' . esc_html((string) ($runResult['exit_code'] ?? '')) . ')</p>';
            echo '<textarea readonly rows="8" class="large-text code">' . esc_textarea($runResult['output'] ?? '') . '</textarea>';
        }
        if ($cleanupResult) {
            echo '<p><strong>Lokien tyhjennys:</strong> ' . (!empty($cleanupResult['ok']) ? 'OK' : 'Virheitä') . '</p>';
            echo '<textarea readonly rows="6" class="large-text code">' . esc_textarea(print_r($cleanupResult, true)) . '</textarea>';
        }
        echo '</div>';
        echo '</div>';
    }
